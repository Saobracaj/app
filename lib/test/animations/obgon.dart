...
class Obgon extends StatefulWidget {
  const Obgon({super.key});

  @override
  State<Obgon> createState() => _ObgonState();
}

class _ObgonState extends State<Obgon> with SingleTickerProviderStateMixin {
      ...
      child: ClipRect(
        child: Stack(
          children: [
            ...
            Positioned.fill(child: const RoadView(moving: true)),

            // Неподвижная машина — синяя, стоит посередине экрана
            Positioned(
              left: width * 0.4,
              bottom: 10,
              child: SizedBox(height: 50, child: AnimatedAutoWidget(color: Colors.blue)),
            ),
