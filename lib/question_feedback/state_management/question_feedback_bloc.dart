...
/// Жалоба на вопрос со вкладки «Обсуждение» или «Конспект».
///
...
/// названием вкладки и ссылкой на вопрос. Ссылку бэкенд превращает во вложение
/// «вопрос N» сам, но id передаётся и явно (сервер их дедуплицирует), чтобы
/// вложение появилось, даже если формат ссылки когда-нибудь поменяется.
///
...
class QuestionFeedbackBloc
  ...
  /// Текст сообщения: маркер `Report`, вкладка, с которой пришла жалоба, ссылка
  /// на вопрос и сам текст пользователя.
  ///
  ...
  String composeBody(String text) => [
    'Report · ${source.key}',
    appLink('/question/$questionId').toString(),
    '',
    text.trim(),
  ].join('\n');
