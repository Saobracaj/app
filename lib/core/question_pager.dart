...
/// с тех пор ею листаются и тренажёр, и симуляция экзамена.
...
/// страницу, блоку об этом сообщают. Страницы, которые программная анимация
/// проходит транзитом, переходами не считаются и не докладываются — блок
/// принял бы доклад за новую цель и вернул листалку назад с полпути (так
/// дальний клик в пагинации оседал на соседнем вопросе). Программный переход
/// не принимают и за жест: [onLeaving] зовут, только когда протяжка пальцем
    ...
    return MediaQuery(
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: PageView.builder(
          ...
          onPageChanged: (index) {
            // Докладывать блоку — только страницы, переведённые пальцем (и
            // прямо под ним, не дожидаясь остановки). Программная анимация
            // проходит промежуточные страницы транзитом: доложи их — блок
            // примет доклад за новую цель и вернёт листалку назад, едва
