...
class _StubKonspektRepository extends KonspektRepository {
  ...
  Future<Konspekt?> load(String categoryId) async => Konspekt(
    ...
    categoryName: const KonspektText(sr: 'Знаци'),
    sections: const [
      KonspektSection(
        id: 'osnove',
...
class _StubFeatureFlagsRepository extends FeatureFlagsRepository {
  ...
  FeatureFlagsSnapshot get snapshot => FeatureFlagsSnapshot.resolve(
    ...
    // Объяснение и конспект — премиальные фичи, без гранта вкладок просто нет.
...
void main() {
  ...
  testWidgets('под конспектом вопроса есть кнопка «Сообщить об ошибке»', (
    tester,
  ) async {
    await tester.pumpWidget(wrap());
    await tester.pumpAndSettle();

    // Переключаемся на вкладку «Конспект» (свёрнутая вкладка — иконка).
    await tester.tap(find.byIcon(Icons.menu_book_outlined));
    await tester.pumpAndSettle();

    expect(
      find.textContaining('Конспект.', findRichText: true),
      findsOneWidget,
    );
    expect(find.byType(ReportProblemButton), findsOneWidget);
    expect(find.text('questionFeedback.report'), findsOneWidget);
