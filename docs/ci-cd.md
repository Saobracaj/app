...
### iOS — нужно заполнить ⚠️

Автоматически достать эти значения не удалось: они выдаются только в веб-кабинетах
Apple под вашей учёткой с 2FA. Ниже — что именно завести и где взять.
Bundle ID приложения — `at.gleb.saobracaj.saobracaj`, Team ID — `BHH5379JU2`
(уже прописаны в workflow как `IOS_BUNDLE_ID` / `APPLE_TEAM_ID`).
