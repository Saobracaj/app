...
| `checkout_step` | шаг покупки | `step` (`purchase_started`/`purchase_completed`/`purchase_cancelled`/`purchase_failed`/`purchases_restored`/`purchase_rejected` — бэкенд отказал окончательно, транзакция завершена/`purchase_stuck_in_queue` — StoreKit не даёт купить, пока висит старая транзакция, запущен restore/`store_purchase_claimed` — тихая сверка со стором перед продажей нашла живую покупку и записала её на этот аккаунт/`store_subscription_elsewhere` — аккаунт стора платит за подписку другого живого аккаунта, кнопки покупки заперты/`lava_started` — нажата оплата российской картой (веб, lava.top)/`lava_redirected` — счёт создан, вкладка ушла на страницу оплаты lava.top/`lava_paid` — страница возврата дождалась оплаты, подписка активирована/`lava_failed` — счёт не создан, оплата не прошла или подтверждение не пришло/`lava_cancelled` — подписка lava.top отменена из приложения), `sku` |
